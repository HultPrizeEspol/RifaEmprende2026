...
let numeroSeleccionado = null;let modoAdmin = false;let cargados = 0;
const PASO = 100;const TOTAL_NUMEROS = 800;
const verificarNumero = async (num) => {
    const docRef = doc(db, "rifa", num.toString());
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
        const data = docSnap.data();const card = document.getElementById(`num-${num}`);
        if (card) {
            card.classList.add('occupied');card.style.backgroundImage = `url('${data.imageUrl}')`;
...
window.cargarMasNumeros = () => {
    ...
    for (let i = cargados + 1; i <= limite; i++) {
        const card = crearUnCuadro(i);
        grid.appendChild(card); verificarNumero(i);  } cargados = limite;};
